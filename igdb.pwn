...
{
    new plName[MAX_PLAYER_NAME], plIP[26], plVers[10], plHWID[80], OtherString[126], nfile[64];
...
{ //here we will record the last date a player was on every time a player disconnects each in their own file.
    ...
    new pyear, pmonth, pday, phour, pminute, psecond, plName[MAX_PLAYER_NAME], FullDate[100], nfile[64];
...
{
    new pidx,ptmp[256], StatsString[1024], plName[MAX_PLAYER_NAME], nfile[64];
    ...
    //down there formatting the string and install it in the msgboxstyle dialogue.
